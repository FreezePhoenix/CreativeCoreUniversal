...
public abstract class GuiCreativeIngredientHandler {
    ...
    private static final PairList<String, GuiCreativeIngredientHandler> handlers = new PairList<>();
    ...
    static {
        ...
        registerGuiInfoHandler("Blocktag", new GuiCreativeIngredientHandler() {
            ...
            public void createControls(GuiParent gui, CreativeIngredient info) {
                ...
                GuiComboBoxMapped<TagKey<Block>> box = new GuiComboBoxMapped<>("tag", new TextMapBuilder<TagKey<Block>>()
                        .addComponents(Registry.BLOCK.getTagNames().toList(), (TagKey<Block> x) -> {
                            return new TextBuilder().stack(new ItemStack(Registry.BLOCK.getTag(x).get().get(0).value())).text(x.location().toString()).build();
                        }));
                gui.add(box);
            ...
            public void onChanged(GuiParent gui, GuiControlChangedEvent event) {
                if (event.control.is("search")) {
                    ...
                    box.setLines(new TextMapBuilder<TagKey<Block>>().setFilter(x -> x.toLowerCase().contains(((GuiTextfield) event.control).getText()))
                            .addComponents(Registry.BLOCK.getTagNames().toList(), (TagKey<Block> x) -> new TextBuilder()
                                    .stack(new ItemStack(Registry.BLOCK.getTag(x).get().get(0).value())).text(x.location().toString()).build()));
                }
            }
        });
        ...
        registerGuiInfoHandler("Itemtag", new GuiCreativeIngredientHandler() {
            ...
            public void createControls(GuiParent gui, CreativeIngredient info) {
                gui.flow = GuiFlow.STACK_Y;
                gui.align = Align.STRETCH;
                GuiComboBoxMapped<TagKey<Item>> box = new GuiComboBoxMapped<>("tag", new TextMapBuilder<TagKey<Item>>().addComponents(Registry.ITEM.getTagNames()
                        .toList(), (TagKey<Item> x) -> new TextBuilder().stack(new ItemStack(Registry.ITEM.getTag(x).get().get(0))).text(x.location().toString()).build()));
                gui.add(box);
                gui.add(new GuiTextfield("search"));
            ...
            public void onChanged(GuiParent gui, GuiControlChangedEvent event) {
                if (event.control.is("search")) {
                    GuiComboBoxMapped<TagKey<Item>> box = (GuiComboBoxMapped<TagKey<Item>>) gui.get("tag");
                    box.setLines(new TextMapBuilder<TagKey<Item>>().setFilter(x -> x.toLowerCase().contains(((GuiTextfield) event.control).getText())).addComponents(Registry.ITEM
                            .getTagNames().toList(), x -> new TextBuilder().stack(new ItemStack(Registry.ITEM.getTag(x).get().get(0))).text(x.location().toString()).build()));
                }
            }
        });
