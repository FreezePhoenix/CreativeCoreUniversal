...
public class OpenGuiPacket extends CreativePacket {
    ...
    public static void openGuiOnServer(GuiCreator creator, CompoundTag nbt, ServerPlayer player) {
        player.openMenu(new SimpleMenuProvider((int id, Inventory inventory, Player x) -> new ContainerIntegration(CreativeCore.GUI_CONTAINER.get(), id, x, creator.function
